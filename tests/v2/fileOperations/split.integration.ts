...
describe("MindeeV2 - Integration - FileOperation - Crop", { timeout: 120000 }, () => {
  ...
  after(() => {
    const file1 = path.join(OUTPUT_DIR, "crop_001.jpg");
    const file2 = path.join(OUTPUT_DIR, "crop_002.jpg");
  ...
  it("extracts crops from image correctly", async () => {
    ...
    const file1Path = path.join(OUTPUT_DIR, "crop_001.jpg");
    const file2Path = path.join(OUTPUT_DIR, "crop_002.jpg");
    ...
    assert.ok(stat1.size >= 3170000 && stat1.size <= 3180000);

    const stat2 = fs.statSync(file2Path);
    assert.ok(stat2.size >= 3210000 && stat2.size <= 3230000);
  });
});
