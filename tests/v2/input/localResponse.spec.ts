...
import { InferenceResponse, LocalResponseV2 } from "@/v2/index.js";
...
async function assertLocalResponse(localResponse: LocalResponseV2) {
...
describe("MindeeV2 - Load Local Response", () => {
  it("should load a string properly.", async () => {
    ...
    await assertLocalResponse(new LocalResponseV2(fileObj));
  });

  it("should load a file properly.", async () => {
    await assertLocalResponse(new LocalResponseV2(filePath));
  });
  ...
  it("should load a buffer properly.", async () => {
    ...
    await assertLocalResponse(new LocalResponseV2(fileBuffer));
  });

  it("should deserialize a prediction.", async () => {
    const fileObj = await fs.readFile(filePath, { encoding: "utf-8" });
    const localResponse = new LocalResponseV2(fileObj);
