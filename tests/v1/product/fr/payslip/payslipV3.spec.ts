...
const dataPath = {
  complete: path.join(V1_PRODUCT_PATH, "payslip_fra/response_v3/complete.json"),
  empty: path.join(V1_PRODUCT_PATH, "payslip_fra/response_v3/empty.json"),
  docString: path.join(V1_PRODUCT_PATH, "payslip_fra/response_v3/summary_full.rst"),
  page0String: path.join(V1_PRODUCT_PATH, "payslip_fra/response_v3/summary_page0.rst"),
};

describe("MindeeV1 - PayslipV3 Object initialization", async () => {
  it("should load an empty document prediction", async () => {
    const jsonData = await fs.readFile(path.resolve(dataPath.empty));
    const response = JSON.parse(jsonData.toString());
    const doc = new mindee.v1.Document(mindee.v1.product.fr.PayslipV3, response.document);
