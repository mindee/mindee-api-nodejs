...
const dataPath = {
  complete: "tests/data/products/international_id/response_v1/complete.json",
  empty: "tests/data/products/international_id/response_v1/empty.json",
  docString: "tests/data/products/international_id/response_v1/summary_full.rst",
  page0String: "tests/data/products/international_id/response_v1/summary_page0.rst",
};

describe("InternationalIdV1 Object initialization", async () => {
  it("should load an empty document prediction", async () => {
    const jsonData = await fs.readFile(path.resolve(dataPath.empty));
    const response = JSON.parse(jsonData.toString());
    const doc = new mindee.Document(mindee.product.InternationalIdV1, response.document);
