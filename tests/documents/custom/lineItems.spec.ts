...
describe("Custom Document Line Items", async () => {
    ...
    fieldNamesToLineItems.push("beneficiary_birth_date", "beneficiary_number", "beneficiary_name", "beneficiary_rank");

    const lineItems = getLineItems(anchorNames,  0.011, fieldNamesToLineItems, doc.fields);

    expect(lineItems).to.not.null;
    expect(lineItems.rows.length).to.be.eq(3);
    let firstLine = lineItems.rows[0];
