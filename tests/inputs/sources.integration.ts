...
describe("Given a client", async () => {
  let client: mindee.Client;
  let filePath: string;

  beforeEach(() => {
    client = new mindee.Client();
    filePath = "tests/data/products/invoices/default_sample.jpg";
  });
  ...
  it("should send a document from url.", async () => {
    const url = "https://raw.githubusercontent.com/mindee/client-lib-test-data/" +
      "refs/heads/main/products/invoice_splitter/invoice_5p.pdf";
